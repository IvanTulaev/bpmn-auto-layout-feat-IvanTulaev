export class Grid {
  ...
  get colCount() {
    return this.getGridDimensions()[1]
  }
  ...
  removeElement(element) {
    if (!element) return;
    if (!this.elements.has(element)) return;
    this.removeElementAt(this.find(element))
  }
  ...
  createRow(afterIndex) {
    const newLength = afterIndex >=0 ? this._grid[afterIndex].length :  this._grid[0].length;
    if (!afterIndex && !Number.isInteger(afterIndex)) {
      this._grid.push([]);
    } else {
      this._grid.splice(afterIndex + 1, 0, Array(newLength));
    }
  }
  ...
  createCol(afterIndex) {
    if (!afterIndex && !Number.isInteger(afterIndex)) {
      ...
      for (const row of this._grid){
        row.splice(afterIndex + 1, 0, null);
      }
    }
  }
  ...
  hasIntermediateElements(firstPosition, lastPosition, onVertical) {
    ...
    if (!onVertical) {
      ...
      for (let col = start + 1; col < end; col++) {
        if (!this.hasElementAt(firstPosition[0], col)) continue;
        return true
      }
      return false;
    } else {
