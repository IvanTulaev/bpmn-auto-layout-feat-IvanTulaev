...
export class Layouter {
  ...
  generateDi(layoutGrid , shift, procDi, inPool = false) {
    ...
    const currentYShift = inPool ? shift + DEFAULT_CELL_HEIGHT / 2 : shift;

    // Step 1: Create DI for all elements
    layoutGrid.elementsByPosition().forEach(({ element, row, col }) => {
      const dis = this
        .handle('createElementDi', { element, row, col, layoutGrid, diFactory, shift: currentYShift })
        .flat();
    ...
    layoutGrid.elementsByPosition().forEach(({ element, row, col }) => {
      const dis = this
        .handle('createConnectionDi', { element, row, col, layoutGrid, diFactory, shift: currentYShift })
        .flat();
