...
export class Layouter {
  ...
  createGridLayout(root) {
    ...
    while (grid.elements.size < elementsWithoutBoundary.length) {
      ...
      this.handleGrid(grid,stack, grid.isFlipped);
  ...
  handleGrid(grid, stack, reverse) {
    while (stack.length > 0) {
      ...
      const nextElements = this.handle('addToGrid', { element: currentElement, grid, stack, reverse });
