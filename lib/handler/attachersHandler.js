...
export default {
  'createElementDi': ({ element, row, col, diFactory, shift, xShift }) => {
    const hostBounds = getBounds(element, row, col, null, shift, xShift);
    ...
    (element.attachers || []).sort((a, b) => {
      ...
      const bounds = getBounds(att, row, col, element, shift, xShift);
  ...
  'createConnectionDi': ({ element, row, col, layoutGrid, diFactory, shift, xShift }) => {
    ...
    return attachers.flatMap(att => {
      ...
      return outgoing.map(out => {
        ...
        const waypoints = connectElements(att, target, layoutGrid, true, shift, xShift);
