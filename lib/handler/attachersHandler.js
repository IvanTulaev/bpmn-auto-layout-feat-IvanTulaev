...
export default {
  'createElementDi': ({ element, row, col, diFactory }) => {
    ...
    const DIs = [];
    (element.attachers || []).sort((a, b) => a.outgoing?.length - b.outgoing?.length).forEach((att, i, arr) => {
  ...
  'createConnectionDi': ({ element, row, col, layoutGrid, diFactory }) => {
    const attachers = (element.attachers || []).sort((a, b) => a.outgoing?.length - b.outgoing?.length);
