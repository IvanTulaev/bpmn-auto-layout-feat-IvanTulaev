...
export default {
  'addToGrid': ({ element, grid, stack }) => {
    ...
    const outgoingFromStack = getOutgoingFromStack(element, grid, stack, newOutgoing?.length > 0)
...
function fixNewCrosses(element, grid, stack, nextElements, skipTopLeftOutgoing, forwardOnlyOutgoing) {
  ...
  const edges = [...outgoingEdges, ...incomingEdges]
      .filter(edge => {
        ...
        if (nextElements && (nextElements.includes(target) || nextElements.includes(source))) return false;
        ...
        // TODO: надо ли обрабатывать малое кольцо? пока пропустим
        // TODO: Пока выключаем?
        //реверс логики
...
function getOutgoingFromStack(element, grid, stack, hasNewOutgoings){
  ...
  const processingElements = [...outgoing].filter(edge => {
    ...
    const {target, targetPosition: [targetRow, targetCol]} = edge
    if (!inStackWithoutOutgoing(target, grid, stack)) return false;
...
function moveTopLeftOutgoingForward(element, grid, stack) {
  ...
  while (processingElements.length > 0) {
    ...
    for (let rowIndex = nextElementRow; rowIndex >= 0; rowIndex-- ){
    ...
    const bottomRightPosition = [nextElementRow, grid.colCount -1]
...
function isTracingForTopLeftMove(toElement, fromElement, grid, unwantedEdge , backward) {
  ...
  const [toElementRow, toElementCol] = grid.find(toElement)
  const [fromElementRow, fromElementCol] = grid.find(fromElement)
  ...
  while (nextElements.length > 0) {
    ...
    const fastTrackingEdges = edges.filter(edge => {
      const {source, target, sourcePosition: [sourceRow, sourceCol], targetPosition: [targetRow, targetCol]} = edge
      // убираем self-loop
