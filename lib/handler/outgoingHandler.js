...
import {getDirection} from '../GridWithEdges.js'
...
function fixNewCrosses(element, grid, stack, forwardOnlyOutgoing) {
  ...
  // получаем исходящие
  const nextElementOutgoingElements = grid.getOutgoingElementsFor(element);
  ...
  for (const targetElement of targetElementsInGrid) {
    ...
    const crossPositions = grid.getPositionsHorizontalCrossedBy(element, targetElement)
  ...
  const nextElementIncomingElements = grid.getIncomingElementsFor(element);
  ...
  for (const sourceElement of sourceElementsInGrid) {
    ...
    let crossPositions = grid.getPositionVerticalCrossedBy(sourceElement, element)
    if (crossPositions.length > 0) {
    ...
    if (elNewPosition[0] === sourcePosX[0]) {
      crossPositions = grid.getPositionsHorizontalCrossedBy(sourceElement, element, grid).filter(crossPos => crossPos[0] === elNewPosition[0] )
      if (crossPositions.length > 0) {
    ...
    crossPositions = grid.getPositionsHorizontalCrossedBy(sourceElement, element, grid)
    if (crossPositions.length > 0) {
      const crossRow = crossPositions[0][0]
      const crossCols = crossPositions.map(item => item[1])
      grid.createRow(crossRow - 1)
...
function getInsertPosition (element, previousElement, grid, isBoundarySource) {
  ...
  let crossGrid = grid.getCrossGrid(grid)
  ...
  if (!previousElement && isBoundarySource){
    ...
    crossGrid = grid.getCrossGrid(grid)
    //костыляем для проверки
    if(crossGrid.length - 1 < position[0]){
      crossGrid.push(Array(crossGrid[0].length))
    }
  ...
  if (previousElement) {
    ...
    // проверяем вертикали
    const outgoing = grid.getOutgoingElementsFor(previousElement)
    ...
    const incoming = grid.getIncomingElementsFor(previousElement);
...
function fixNewVerticalCrosses(source, target, grid) {
  ...
  const path = grid.getPathSegments(source, target)
