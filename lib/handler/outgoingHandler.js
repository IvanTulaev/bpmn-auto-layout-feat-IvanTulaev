...
function isFromHost(hostElement, targetElement, reverse) {
  ...
  if (!incoming) throw new Error(`${targetElement.id} has no incoming ${hostElement.id}`);
...
function getInsertPosition(element, previousElement, grid, isBoundarySource) {
  ...
  if (!previousElement && !isBoundarySource) {
    ...
    // костыль до переписывания алгоритма (#41)
    if ([ ...grid._allEdges ].filter(edge => edge.path.some(segment => segment.position[0] === position[0] && segment.position[1] === position[1] && segment.hCross)).length > 0 && grid.isFlipped) {
  ...
  if (previousElement) {
    ...
    // если попали в ребро element-prev, то спускаемся ниже (быстрый фикс #41)
    if ([ ...grid._allEdges ].filter(edge => edge.source === element && edge.target === previousElement && edge.path.some(segment => segment.position[0] === position[0] && segment.position[1] === position[1] && segment.hCross)).length > 0) {
      position[0] = position[0] + 1;
    }
    ...
    for (const edge of incomingEdges) {
      ...
      // быстрый частичный фикс #43
      if (edge.path.some(segment => segment.position[0] === position[0] && segment.position[1] === position[1] && segment.vCross)) {
