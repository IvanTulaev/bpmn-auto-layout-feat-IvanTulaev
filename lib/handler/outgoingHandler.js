...
function fixNewCrosses(element, grid, stack, forwardOnlyOutgoing) {
  ...
  for (const edge of filteredOutgoingEdges) {
    ...
    if (crossPositions.length > 0) {
      ...
      let elementOutgoingEdges = grid.outgoing.get(element) ?? []
      ...
      if (edgesWithTargets.length > 0) {
        ...
        for (const targetEdge of edgesWithTargets) {
          ...
          // const [, tarCol] = targetEdge.tagetPosition;
      ...
      if (crossEls.length > 0) {
        ...
        for (const notTarget of crossEls) {
          // const {target, targetPosition: [, tarCol]} = edge;
  ...
  const incomingEdges = grid.incoming.get(element) ?? []
...
function getInsertPosition (element, previousElement, grid, isBoundarySource) {
  ...
  if (previousElement) {
    ...
    const outgoing = grid.outgoing.get(previousElement) ?? []
    ...
    for (const outgoingEdge of outgoing) {
      ...
      // const targetPosition = grid.find(outgoingElement)
      // const direction = getDirection(previousPosition, targetPosition)
    ...
    const incomingEdges = grid.incoming.get(previousElement) ?? [];
...
function getFirstLeftOutgoingElement(element, grid, stack) {
  ...
  const existingEdges = grid.outgoing.get(element)
  ...
  const processingElements = existingEdges.filter(edge => {
        ...
        if (targetPosition[0] === sourcePosition[0]) return false
        if (targetPosition[1] > sourcePosition[1]) return false;
        if (targetPosition[0] > sourcePosition[0]) return false;
