...
export function getAllUnexpandedSubProcesses(bpmnModel) {
  const allElements = bpmnModel.elementsById;
  if (!allElements) return [];

  const diagramElements = Object.values(allElements)
    .filter(element => element.$type === 'bpmndi:BPMNShape' && element.isExpanded === true).map(element => {

      element.bpmnElement.isExpanded = true;
      return element.bpmnElement;
    });
