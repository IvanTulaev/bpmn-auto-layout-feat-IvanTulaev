...
export function connectElements(source, target, layoutGrid, isBoundary = false, shift) {
  ...
  const { x: targetX, y: targetY } = coordinatesToPosition(target, shift);

  const baseSource = !isBoundary ? source : source.attachedToRef;
  ...
  if (isBoundary) {
    ...
    if (dX === 0 && dY === 0) {
      return [
        ...
        { x: sourceMid.x, y: !source.attachedToRef.isExpanded ? sourceY + DEFAULT_CELL_HEIGHT : sourceY + (source.attachedToRef.grid.rowCount + 2) * DEFAULT_CELL_HEIGHT },
        { x: targetX, y: !source.attachedToRef.isExpanded ? sourceY + DEFAULT_CELL_HEIGHT : sourceY + (source.attachedToRef.grid.rowCount + 2) * DEFAULT_CELL_HEIGHT },
    ...
    if (dX === 0 && dY < 0) {
      return [
        getDockingPoint(sourceMid, sourceBounds, 'b', dockingSource),
        { x: sourceMid.x, y: !source.attachedToRef.isExpanded ? sourceY + DEFAULT_CELL_HEIGHT : sourceY + (source.attachedToRef.grid.rowCount + 2) * DEFAULT_CELL_HEIGHT },
        { x: sourceX, y: !source.attachedToRef.isExpanded ? sourceY + DEFAULT_CELL_HEIGHT : sourceY + (source.attachedToRef.grid.rowCount + 2) * DEFAULT_CELL_HEIGHT },
    ...
    if (dX > 0 && dY < 0) {
      return [
        getDockingPoint(sourceMid, sourceBounds, 'b', dockingSource),
        { x: sourceMid.x, y: !source.attachedToRef.isExpanded ? sourceY + DEFAULT_CELL_HEIGHT : sourceY + (source.attachedToRef.grid.rowCount + 2) * DEFAULT_CELL_HEIGHT },
        { x: targetMid.x, y: !source.attachedToRef.isExpanded ? sourceY + DEFAULT_CELL_HEIGHT : sourceY + (source.attachedToRef.grid.rowCount + 2) * DEFAULT_CELL_HEIGHT },
    ...
    if (dX === 0 && dY === 0) {
      ...
      return [
        getDockingPoint(sourceMid, sourceBounds, 'b', dockingSource),
        { x: sourceMid.x, y: !source.isExpanded ? sourceY + DEFAULT_CELL_HEIGHT : sourceY + (source.grid.rowCount + 2) * DEFAULT_CELL_HEIGHT },
        { x: targetX, y: !source.isExpanded ? sourceY + DEFAULT_CELL_HEIGHT : sourceY + (source.grid.rowCount + 2) * DEFAULT_CELL_HEIGHT },
        { x: sourceX, y: targetMid.y },
        getDockingPoint(targetMid, targetBounds, 'l', dockingTarget)
      ];
    }
    ...
    if (dX === 0 && dY > 0) {
      ...
      if (source.isExpanded || target.isExpanded) {
        return [
          firstPoint,
          { x: sourceMid.x, y: !source.isExpanded ? sourceY + DEFAULT_CELL_HEIGHT : sourceY + (source.grid.rowCount + 2) * DEFAULT_CELL_HEIGHT },
          { x: targetMid.x, y: !source.isExpanded ? sourceY + DEFAULT_CELL_HEIGHT : sourceY + (source.grid.rowCount + 2) * DEFAULT_CELL_HEIGHT },
          lastPoint
        ];
      }
      return [
