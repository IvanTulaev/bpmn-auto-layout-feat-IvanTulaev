...
export function connectElements(source, target, layoutGrid, isBoundary) {
  ...
  if (isBoundary) {
    ...
    if (dX < 0 && dY === 0){
      ...
      let hasSW_NEOut = layoutGrid?.outgoing.get(target) ?? []
...
export function getBounds(element, row, col, attachedTo, shift) {
  ...
  const hostBounds = getBounds(attachedTo, row, col, null, shift);
