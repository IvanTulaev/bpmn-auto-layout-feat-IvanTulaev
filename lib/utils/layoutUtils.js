 ...
 * @returns waypoints
 */
export function connectElements(source, target, layoutGrid, isBoundary = false, shift = 0, xShift = 0) {
  ...
  const { x: sourceX, y: sourceY } = coordinatesToPosition(source.gridPosition.row, source.gridPosition.col, shift, xShift);
  const { x: targetX, y: targetY } = coordinatesToPosition(target.gridPosition.row, target.gridPosition.col, shift, xShift);
...
// helpers /////
export function coordinatesToPosition(row, col, shift, xShift) {
  return {
    width: DEFAULT_CELL_WIDTH,
    height: DEFAULT_CELL_HEIGHT,
    x: col * DEFAULT_CELL_WIDTH + xShift,
    y: row * DEFAULT_CELL_HEIGHT + shift
  };
}
...
export function getBounds(element, row, col, attachedTo, shift, xShift) {
  ...
  if (attachedTo.isExpanded && attachedTo.grid) {
    const { rowCount = 1, colCount = 2 } = attachedTo.grid;
    hostBounds.width = colCount * DEFAULT_CELL_WIDTH + DEFAULT_CELL_WIDTH;
    hostBounds.height = rowCount === 0 ? 1 * DEFAULT_CELL_HEIGHT + DEFAULT_CELL_HEIGHT : rowCount * DEFAULT_CELL_HEIGHT + DEFAULT_CELL_HEIGHT;
  }
